    ...
    public class CuckooHash<K, V> {
        ...
        public void put(K key, V value) {
            ...
            while (attempts < CAPACITY) {
                // First, check if the key-value pair already exists at position1
                if (table[position1] != null && table[position1].getBucKey().equals(key) && table[position1].getValue().equals(value)) {
                    return; // The key-value pair is already in the table; no insertion needed
                }

                if (table[position1] == null) {
                    table[position1] = new Bucket<>(key, value);
                    return; // Successfully inserted
                }

                Bucket<K, V> temp = table[position1];
                table[position1] = new Bucket<>(key, value);
                key = temp.getBucKey();
                value = temp.getValue();

                // Now attempt to place the displaced item in position2
                if (table[position2] == null) {
                    table[position2] = new Bucket<>(key, value);
                    return; // Successfully inserted
                }

                temp = table[position2];
                table[position2] = new Bucket<>(key, value);
                key = temp.getBucKey();
                value = temp.getValue();

                // Move back to position1 for the next iteration
                position1 = hash1(key);
                position2 = hash2(key);
                attempts++;
            }
